...
public actor FeatureIndexCache {
    private var cache: [UInt16: UInt8] = [:]

    public init() {}

    /// Get cached index for a feature ID. Returns nil if not cached.
    public func get(_ featureId: UInt16) -> UInt8? {
        cache[featureId]
    }

    /// Store a feature ID -> index mapping.
    public func set(_ featureId: UInt16, index: UInt8) {
        cache[featureId] = index
    }

    /// Store multiple mappings at once.
    public func setAll(_ mappings: [(featureId: UInt16, index: UInt8)]) {
    ...
    /// All cached entries.
    public var all: [UInt16: UInt8] {
        cache
    }

    /// Copy all entries from another cache (used when promoting a probe to a typed device).
    public func transferFrom(_ other: FeatureIndexCache) async {
